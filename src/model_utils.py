...
from audioldm import text_to_audio, style_transfer, build_model,save_wave,\
      get_time, round_up_duration, get_duration, super_resolution_and_inpainting
...
class AudioLdmWrapper:
    ...
    def text_to_audio(self, text, file_path, duration, guidance_scale, ddim_steps, n_candidate_gen_per_text, save_path=None):
    ...
    def audio_style_transfer(self, text, file_path, transfer_strength, duration, guidance_scale, ddim_steps, save_path=None):
        if not self.is_ready_:
            return False
        self.is_ready_ = False
    ...
    def audio_super_resolution(self, text, file_path, duration, guidance_scale, ddim_steps, n_candidate_gen_per_text, 
                               time_mask_ratio_start_and_end, freq_mask_ratio_start_and_end, save_path=None):
        if not self.is_ready_:
            return False
        self.is_ready_ = False
...
@st.cache_resource
def init_model(img_model_name="nlpconnect/vit-gpt2-image-captioning", img_model_end='huggingface',\
                audio_model_name='audioldm-s-full-v2',audio_model_ckpt='./assets/audioldm-full-s-v2.ckpt'):
...
if __name__ == "__main__":
    save_path='./output/'
    audioldm=init_img2text_model('./data/ldm_trimmed.ckpt')
