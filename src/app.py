...
image2text_model, audio_tool = init_model(img2text_model_name,img2text_model_backend,\
                                    audio_model_name,audio_model_ckpt)
...
if musician_data=='Image':
    with st.container():
        ...
        if uploaded_img is not None:
            ...
            if global_is_ready:
                ...
                audio_imagined = audio_tool.text_to_audio(img_caption, None, duration=duration, guidance_scale=guidance_scale,\
                                      ddim_steps=ddim_steps, n_candidate_gen_per_text=n_candidate,save_path=save_path)
                ...
                audio_imagined_styled2=audio_tool.audio_style_transfer('piano music',file_path=save_path+'audio_0.wav',\
                                      transfer_strength=transfer_strength,duration=duration, guidance_scale=guidance_scale,\
                                      ddim_steps=ddim_steps,save_path=save_path)
    ...
    with st.container():
        ...
        if global_is_ready:
            ...
            audio_imagined = audio_tool.text_to_audio(text, None, duration=duration, guidance_scale=guidance_scale,\
                                    ddim_steps=ddim_steps, n_candidate_gen_per_text=n_candidate,save_path=save_path)
            if not isinstance(audio_imagined,bool):
                file_0 = get_wav_audio(save_path+'audio_0.wav')
                st.audio(file_0, format='audio/wav') # sample_rate=sample_rate
            ...
            torch.cuda.empty_cache()
            audio_imagined_styled = audio_tool.audio_style_transfer('piano music',file_path=save_path+'audio_0.wav',\
                                    transfer_strength=transfer_strength,duration=duration, guidance_scale=guidance_scale,\
                                    ddim_steps=ddim_steps,save_path=save_path)
